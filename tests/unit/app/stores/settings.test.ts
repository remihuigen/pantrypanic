...
describe('useSettingsStore', () => {
	...
	it('switches household and refreshes every settings section', async () => {
		...
		const calls: string[] = []
		let resolveProfile: ((_value: { user: ReturnType<typeof profile> }) => void) | null = null
		const profilePromise = new Promise<{ user: ReturnType<typeof profile> }>((_resolve) => {
			resolveProfile = _resolve
		})

		store.members = [member()]
		store.householdSettings = householdSettings()
		store.items = [settingsItem()]
		store.categories = [settingsCategory()]
		store.stats = stats()
		store.inviteUrl = '/invite/old'
		store.resetUrl = '/reset/old'
		listsStore.listsById['list-1'] = createList()
		listsStore.activeListIds = ['list-1']
		recipesStore.recipesById['recipe-1'] = createRecipe()
		recipesStore.activeRecipeIds = ['recipe-1']
		mealPlannerStore.mealPlannerDaysById['day-1'] = createMealPlannerDay()
		mealPlannerStore.mealPlannerDayIds = ['day-1']
		...
		const switchPromise = store.switchHousehold('work')
		...
		expect(store.members).toEqual([])
		expect(store.householdSettings).toBeNull()
		expect(store.items).toEqual([])
		expect(store.categories).toEqual([])
		expect(store.stats).toBeNull()
		expect(store.inviteUrl).toBeNull()
		expect(store.resetUrl).toBeNull()
		expect(listsStore.activeListIds).toEqual([])
		expect(listsStore.listsById).toEqual({})
		expect(recipesStore.activeRecipeIds).toEqual([])
		expect(recipesStore.recipesById).toEqual({})
		expect(mealPlannerStore.mealPlannerDayIds).toEqual([])
		expect(mealPlannerStore.mealPlannerDaysById).toEqual({})

		resolveProfile?.({ user: profile() })
		await switchPromise
...
function createList(overrides: Partial<{ id: string; name: string; position: number }> = {}) {
	return {
		id: 'list-1',
		name: 'Boodschappen',
		icon: 'lucide:list',
...
function createRecipe(overrides: Partial<{ id: string; name: string; updatedAt: number }> = {}) {
	return {
		id: 'recipe-1',
		name: 'Pasta',
		status: 'active' as const,
...
function createMealPlannerDay(
	overrides: Partial<{ id: string; dayOfWeek: number; updatedAt: number }> = {}
) {
	return {
		id: 'day-1',
		dayOfWeek: 1,
