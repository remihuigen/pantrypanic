...
Set `behavior: smooth` to enable smooth scrolling. The `behavior` option defaults to `auto`, which
means no smooth scrolling. See the `behavior` option on
...
You can call the `measure()` method to manually update the scroll position and `arrivedState` at any
time.

This is useful, for example, after dynamic content changes or when you want to recalculate the
scroll state outside of scroll events.
...
> [!NOTE] it's recommended to call `measure()` inside `nextTick()`, to ensure the DOM is updated
> first. The scroll state is initialized automatically `onMount`. You only need to call `measure()`
