...
If you need more control over triggering the handler, you can use the `controls` option. This
...
Use the `ignore` option to prevent certain elements from triggering the handler. Provide elements as
an array of Refs or CSS selectors.
...
By default, the event listener uses the capture phase (`capture: true`). Set `capture: false` to use
the bubbling phase instead.
...
Clicks inside an iframe are not detected by default. Enable `detectIframe` to also trigger the
