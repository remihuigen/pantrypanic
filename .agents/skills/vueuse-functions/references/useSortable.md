...
[`Sortable.options`](https://github.com/SortableJS/Sortable#options) in the `Sortable`
documentation.
...
Use the `watchElement` option to automatically reinitialize Sortable when the element changes
(useful with `v-if`).
...
If you want to handle the `onUpdate` yourself, you can pass in `onUpdate` parameters, and we also
exposed a function to move the item position.
