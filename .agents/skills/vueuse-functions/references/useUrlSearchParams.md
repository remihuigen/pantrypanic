...
When using with history mode route, but want to use hash as params, specify the `mode` to
`hash-params`
...
You can provide a custom function to serialize URL parameters using the `stringify` option. This is
useful when you need special formatting for your query string.
