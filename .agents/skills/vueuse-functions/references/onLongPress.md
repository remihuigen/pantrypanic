...
By default, the handler fires after 500ms. You can customize this with the `delay` option. It can be
a number or a function that receives the `PointerEvent`.
...
<template>
  ...
  <OnLongPress as="button" class="button small ml-2" @trigger="onLongPressCallbackComponent">
    Press long
  </OnLongPress>
...
<template>
  ...
  <button v-on-long-press.prevent="onLongPressCallbackDirective" class="button small ml-2">
    Press long
  </button>
