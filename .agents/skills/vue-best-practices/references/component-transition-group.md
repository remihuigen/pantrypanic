...
impact: MEDIUM
impactDescription:
  TransitionGroup animates list items; missing keys or misuse leads to broken list transitions
type: best-practice
...
**Impact: MEDIUM** - `<TransitionGroup>` animates lists of items entering, leaving, and moving. Use
it for `v-for` lists or dynamic collections where individual items change over time.
...
`<TransitionGroup>` is designed for list items. Use `tag` to control the wrapper element when
needed.
...
`mode` is only for `<Transition>` because it swaps a single element. Use `<Transition>` if you need
in/out sequencing.
