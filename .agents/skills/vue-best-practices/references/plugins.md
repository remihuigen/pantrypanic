...
impact: MEDIUM
impactDescription:
  Incorrect plugin structure or injection key strategy causes install failures, collisions, and
  unsafe APIs
type: best-practice
...
**Impact: MEDIUM** - Vue plugins should follow the `app.use()` contract, expose explicit
capabilities, and use collision-safe injection keys. This keeps plugin setup predictable and
composable across large apps.
...
String keys can collide (`'http'`, `'config'`, `'i18n'`). Use symbol keys with `InjectionKey<T>` so
injections are unique and typed.
