...
impact: CRITICAL
description:
  Default z.object() strips unknown keys. Use strictObject or looseObject for explicit behavior.
...
`z.object()` silently strips any keys not defined in the schema. This can cause data loss when
forwarding payloads, storing extra metadata, or proxying API responses.
