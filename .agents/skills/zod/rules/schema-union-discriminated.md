...
`z.union()` tries each branch sequentially until one matches. For object unions with a shared
discriminator field (like `type` or `kind`), this is slow and produces confusing error messages that
show issues from every branch.
...
`discriminatedUnion` checks the discriminator field first, then only validates the matching branch.
This gives O(1) dispatch instead of O(n) sequential checking, and error messages target the actual
problem instead of listing every branch's failures.
