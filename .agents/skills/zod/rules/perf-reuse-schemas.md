...
Duplicating field definitions across related schemas (Create, Update, Response) leads to drift. When
a field changes, only some schemas get updated.
...
`pick()`, `omit()`, `partial()`, and `required()` derive new schemas from a base. Changes to the
base propagate automatically to all derived schemas. The inferred types also stay in sync.
