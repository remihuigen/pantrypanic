...
impact: MEDIUM
description:
  Use v4 error parameter everywhere. message, required_error, invalid_type_error, errorMap removed.
...
Zod v3 had multiple ways to customize errors: `message`, `required_error`, `invalid_type_error`, and
`errorMap`. Zod v4 replaces all of them with a single `error` parameter.
...
The unified `error` parameter simplifies error customization. The string form covers most cases. The
function form provides access to the issue context (`input`, `code`, `minimum`, etc.) for dynamic
messages. Global error customization uses `z.config()` with an error map function.
