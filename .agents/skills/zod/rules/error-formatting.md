...
impact: HIGH
description:
  Use z.treeifyError() for nested, z.flattenError() for forms. Not deprecated z.formatError().
...
Zod v4 replaces `z.formatError()` with `z.treeifyError()` and `z.flattenError()`. Using the
deprecated function gives different output shapes.
