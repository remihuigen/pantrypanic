...
impact: MEDIUM
description:
  Use spread syntax with .shape over .extend() for large schemas to reduce compile-time cost.
...
`.extend()` creates a new schema type at the TypeScript level, which is expensive at compile time.
Chaining multiple `.extend()` calls compounds this cost.
...
`.extend()` is fine for one-off extensions. But when building large type hierarchies, spreading
`.shape` keeps compile times manageable. The runtime behavior is identical — this is purely a
