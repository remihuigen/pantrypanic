...
`parse()` throws a `ZodError` on invalid input. Wrapping it in try/catch is verbose and error-prone
— you lose the discriminated success/error result and risk catching unrelated errors.
...
No exceptions, no try/catch, no risk of catching unrelated errors. Use `parse()` only when invalid
data is truly exceptional (e.g., internal config that should never be wrong).
