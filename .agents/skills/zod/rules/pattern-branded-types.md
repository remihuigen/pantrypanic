...
you can accidentally pass a USD value where EUR is expected, or a UserId where a PostId is expected.
...
`.brand()` adds a phantom type brand that makes structurally identical types incompatible at the
TypeScript level. Use this for IDs (UserId vs PostId), currencies, units, or any domain where mixing
values is a logic error.
