...
Complex validation chains that coerce and validate in multiple stages become messy when done in a
single transform. Each stage should parse the output of the previous stage.
...
`.pipe()` feeds the output of one schema into another. Each stage produces proper Zod errors on
failure. This is cleaner than manual transforms with throws, and each stage's type is properly
inferred.
